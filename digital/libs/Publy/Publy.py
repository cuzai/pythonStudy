...
class Publy(QThread) :
    ...
    def run(self):
        try :
            ...
            with webdriver.Chrome(chrome_options = self.chromeOptions, executable_path = './webdriver/chrome/chromedriver') as driver:
                # self.driver = webdriver.Chrome(chrome_options = self.chromeOptions, executable_path = '../../webdriver/chrome/chromedriver')
                driver.get(self.url)
                driver.implicitly_wait(10)
                while(True) :
                    temp = driver.find_elements_by_css_selector('.set-list__container.curation.bg-white.pt-7')[divNum]
            ...
            logging.info(">>>>> Pybly error : {}, {}".format(e, sys.exc_info()))
